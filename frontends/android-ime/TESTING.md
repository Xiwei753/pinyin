...
- [ ] *故障排查：如果出现了候选，但第一候选是“妳好”、“逆號”等繁体/异体字，这说明部署已成功，问题在于简体过滤。请确保使用的是已配置 `simplifier` 和 `uniquifier` 的最新包。*
...
- [ ] 方案列表中存在“希为九宫格”方案。
- [ ] 切换至“希为九宫格”方案，不会引发崩溃。
- [ ] *（注：Fcitx5 Android 的键盘 UI 不会因为切换 Rime schema 而变成九键。因此，这里的“希为九宫格”仅作为数字串匹配实验。真正的九键需要等待未来的原生 Android 前端开发。）*
...
4. **九键只是实验方案**：由于 Fcitx5 Android 的键盘布局是硬编码的，Rime 方案无法改变 UI。目前九键仅作纯后端数字映射验证。
