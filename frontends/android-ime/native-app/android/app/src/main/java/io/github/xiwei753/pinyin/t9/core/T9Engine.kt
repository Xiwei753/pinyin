...
class T9Engine(
    ...
    fun getVisibleCandidates(limit: Int = 30): List<Candidate> {
        ...
        val userTexts = userCands.map { it.text }.toSet()
        ...
        // Remove the block on generating prefixPhrases so partial matches can be displayed mid-typing!
        val prefixPhrases = if (!hasExact) {
        ...
        // Ensure EXACT phrases beat dynamic compositions if any, though dynamic should be empty here based on the boolean check.
        // We boost EXACT phrases so that they outrank prefix completions.
        val boostedExactPhrases = exactPhrases.map { it.copy(score = it.score + 10000000) }
        val boostedExactSingles = exactSingles.map { it.copy(score = it.score + 500000) }
        val deflatedDynamic = dynamicFallback.map { it.copy(score = it.score / 1000) }

        // Increase user candidate score so it properly boosts over exact singles
        val boostedUserCands = userCands.map { it.copy(score = it.score + 20000000) }
        val filteredExactSingles = boostedExactSingles.filter { it.text !in userTexts }
        val combined = (boostedExactPhrases + boostedUserCands + filteredExactSingles + deflatedDynamic + prefixPhrases + prefixSingles)
