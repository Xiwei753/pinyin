...
- 但如果此时输入 `nihao` 只有预编辑（即只显示拼音字母），没有出现“你好”等中文候选词，这说明是 Rime 配置/词库候选生成的问题。
- 解决办法：
  1. 确保你使用的是更新后包含修复词库和配置的新版本（本项目已修复此问题）。
  2. 请重新回到 Actions 页面，下载最新的 `android-rime-fcitx5-userdata`。
  3. 按照第 2 步，重新导入最新的用户数据包。
  4. 务必再次进入设置点击 **重新部署** (Deploy) 或彻底重启 Fcitx 实例。
  5. 也可以在键盘方案中切换到内置兜底的“明月拼音·简化字” (`luna_pinyin_simp`) 方案来测试环境是否正常。如果兜底方案能出候选词，而“希为拼音”依然不行，说明 Rime 引擎正常工作，可能是文件配置问题；如果兜底方案也不行，请重点检查候选窗口设置、重试清理缓存或彻底重启 Fcitx。详细步骤请参考 `docs/android-rime-no-candidates-debug.md` 和 `docs/fcitx5-android-rime-deploy-debug.md`。
