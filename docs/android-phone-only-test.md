...
- 解决办法：
  ...
  5. 也可以在键盘方案中切换到内置兜底的“明月拼音·简化字” (`luna_pinyin_simp`) 方案来测试环境是否正常。如果兜底方案能出候选词，而“希为拼音”依然不行，说明 Rime 引擎正常工作，可能是文件配置问题；如果兜底方案也不行，请重点检查候选窗口设置、重试清理缓存或彻底重启 Fcitx。详细步骤请参考 `docs/android-rime-no-candidates-debug.md`。
