...
我们当前的重点转向：**九键输入验证**。
目标是让“希为九宫格”能被添加和切换，并且在输入数字序列（如 `64426`）时能正常映射并输出对应的中文（如“你好”）。如果 Fcitx5 Android 的 Rime 九键方案短期不稳定，将明确标记为实验性方案，不强求完美的日用打字体验，主要验证基础映射逻辑。
...
6. **注意**：希为九宫格目前是实验方案。当前阶段致力于验证在 Android 上的基础 9 键映射配置能否生效，并不完全成熟，请勿强求日用效果。
